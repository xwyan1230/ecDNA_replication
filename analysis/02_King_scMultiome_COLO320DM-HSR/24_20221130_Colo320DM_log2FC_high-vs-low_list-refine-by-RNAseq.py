...
print(len(df))
gene_log2FC_high = pd.read_csv('%s%s_average-expression_enrich_in_C2_log2FC_low_gene-location.txt' % (data_dir, prefix), na_values=['.'], sep='\t')
...
gene_log2FC_high.to_csv('%s%s_average-expression_enrich_in_C2_log2FC_low.txt' % (output_dir, prefix), index=False, sep='\t')
