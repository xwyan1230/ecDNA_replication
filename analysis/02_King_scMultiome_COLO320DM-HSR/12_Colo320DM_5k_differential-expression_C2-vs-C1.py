...
df = pd.read_csv('%s%s_average-expression.txt' % (data_dir, prefix), na_values=['.'], sep='\t')
print(len(df))
...
df_C2enrich.to_csv('%s%s_average-expression_enrich_in_C2.txt' % (output_dir, prefix), index=False, sep='\t')
df_C1enrich = df[df['C2/C1+C2'] < 0.5].sort_values(by=['gamma_C2/C1'], ascending=False)
print(len(df_C1enrich))
df_C1enrich.to_csv('%s%s_average-expression_enrich_in_C1.txt' % (output_dir, prefix), index=False, sep='\t')
...
# 2d volcano plot
fig, ax = plt.subplots()
plt.scatter(df['C2/C1+C2'], df['p_C2/C1'], c='black', s=5, alpha=0.5)
