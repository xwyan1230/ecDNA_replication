...
df1 = pd.read_csv('%s%s_normalized_average_expression_enrich_in_C2_log2FC_low_global.txt' % (data_dir, prefix), na_values=['.'], sep='\t')
df2 = pd.read_csv('%s%s_normalized_average_expression_enrich_in_C2_log2FC_high_local_C1_summary.txt' % (data_dir, prefix), na_values=['.'], sep='\t')
...
ax.scatter(df1['start'][:50], 25-df1['chr_num'][:50], c='r', alpha=0.3)
...
plt.savefig('%sfigures/%s_chr_location_C2_log2FC_low_global_top50.pdf' % (output_dir, prefix))
plt.close()
